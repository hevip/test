    ...
    UserInfoControllerInterface
{
     ...
     * @param $key
     * If null, the storage is set to the key of each storage interfaces it implements
     *
     * @see storageMap
     */
    public function addStorage($storage, $key = null)
    {
        ...
        if (isset($this->storageMap[$key])) {
            if (!is_null($storage) && !$storage instanceof $this->storageMap[$key]) {
                throw new \InvalidArgumentException(sprintf('storage of type "%s" must implement interfaces "%s"', $key, $this->storageMap[$key]));
            }
    ...
    public function addResponseType(ResponseTypeInterface $responseType, $key = null)
    {
        ...
        if (isset($this->responseTypeMap[$key])) {
            if (!$responseType instanceof $this->responseTypeMap[$key]) {
                throw new \InvalidArgumentException(sprintf('responseType of type "%s" must implement interfaces "%s"', $key, $this->responseTypeMap[$key]));
            }
